...
// You can try more different parameters
	...
	if (height> width){
		height=20;
		const sf = image.rows/height;
		width = Math.round(image.cols/sf);


	}
	else{
		width=20;
		const sf = image.cols/width;
		height= Math.round(image.rows/sf);
	...
	let pixelValue = image.data; //returns an array of integer number

	// console.log(`pixel value is: ${pixelValue}`);

	pixelValue = Float32Array.from(pixelValue);


	pixelValue = pixelValue.map(function(pixels){
	...
	// console.log(`Scaled values: ${pixelValue}`);

	const X = tf.tensor([pixelValue]);
	// console.log(`shape of X : ${X.shape}`);
	// console.log(`shape of Y: ${X.dtype}`);
